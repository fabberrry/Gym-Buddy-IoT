...
namespace
{
    std::string uniqueSessionId()
    {
        ...
        const auto pid = static_cast<std::uint64_t>(
#ifdef _WIN32
            GetCurrentProcessId()
#else
            getpid()
...
int main(int argc, char **argv)
{
    try
    {
        ...
        initial.deviceId = "counter-01";
        initial.roomId = "room-01";
        initial.sessionId = uniqueSessionId();
        ...
        for (int i = 1; i < argc; ++i)
        {
            ...
            if (arg == "--device-id")
                initial.deviceId = value;
            else if (arg == "--room-id")
                initial.roomId = value;
        ...
        for (std::size_t i = 0; live || i < frames.size(); ++i)
        {
            ...
            if (live)
                f.time_ms = std::uint64_t(std::chrono::duration_cast<std::chrono::milliseconds>(
            ...
            {
                const char *event = result.detection.outcome == doorway::Outcome::Enter ? "ENTRY" : result.detection.outcome == doorway::Outcome::Exit ? "EXIT"
                                                                                                                                                       : "AMBIGUOUS";
